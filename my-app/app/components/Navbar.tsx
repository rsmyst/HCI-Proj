        ...
        <div className="hidden lg:flex items-center gap-2">
          ...
          {currentUser ? (
            <>
              <Link
                href="/dashboard"
                className="border border-white/30 text-white text-sm px-4 py-2 rounded-md hover:bg-white/10 active:bg-white/20 transition-all duration-150"
              >
                Dashboard
              </Link>
              <button
        ...
        <div className="lg:hidden bg-[#132f57] dark:bg-gray-950 border-t border-white/10 px-6 py-4 flex flex-col gap-3 text-sm animate-slide-down transition-colors duration-300">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="hover:text-orange-300 py-1 transition-colors duration-150"
            >
              {link.label}
            </a>
          ))}
          <div className="pt-2 border-t border-white/10">
            <div className="text-xs uppercase text-blue-200 dark:text-gray-400 tracking-wider mb-2">
              More
            </div>
            {moreLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="block py-1 text-white/90 hover:text-orange-300 transition-colors"
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="flex gap-2 pt-2 border-t border-white/10">
            {currentUser ? (
              <>
                <Link
                  href="/dashboard"
                  className="flex-1 border border-white/30 text-white py-2 rounded-md hover:bg-white/10 transition-colors text-center"
                >
                  Dashboard
                </Link>
                <button
                  type="button"
                  onClick={handleLogout}
                  className="flex-1 bg-orange-500 text-white py-2 rounded-md font-medium hover:bg-orange-600 transition-colors text-center"
                >
                  Logout
                </button>
              </>
