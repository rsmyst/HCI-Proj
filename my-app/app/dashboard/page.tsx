          ...
          <div className="px-6 py-8 sm:px-10 sm:py-10 grid gap-6 md:grid-cols-[1.4fr_0.9fr] items-start">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm sm:text-base text-blue-100">
                Continue booking, search for a different train, or jump back
                into the itinerary you were already exploring.
              </p>

              <div className="mt-6 flex flex-wrap gap-3">
                <Link
                  href="/search"
                  className="rounded-xl bg-white px-5 py-3 text-sm font-semibold text-[#1a3c6e] transition-transform hover:scale-[1.02]"
                >
                  Search trains
                </Link>
                <Link
                  href="/booking"
                  className="rounded-xl border border-white/30 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-white/10"
                >
                  Continue booking
                </Link>
                <button
                  ...
                  className="rounded-xl bg-orange-500 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-orange-600"
                >
                  Logout
                </button>
              </div>
              ...
              <dl className="mt-4 space-y-3 text-sm">
                ...
                <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-2">
                  ...
                  <dd className="font-medium">
                    +{currentUser.countryCode.replace("+", "")}{" "}
                    {currentUser.mobile}
                  </dd>
                </div>
            ...
            {upcomingBookings.length === 0 ? (
              ...
              <ul role="list" className="mt-4 space-y-3">
                {upcomingBookings.map((booking) => (
                  <li
                    key={booking.id}
                    className="rounded-xl border border-slate-200 dark:border-gray-700 bg-slate-50 dark:bg-gray-800/60 p-4"
                  >
        ...
        <section className="grid gap-4 md:grid-cols-2">
          <article className="rounded-2xl border border-slate-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-600 dark:text-gray-400">
              Change source, destination, or date before choosing a different
              train.
            </p>
            <Link
              href="/search"
              className="mt-5 inline-flex rounded-xl bg-[#1a3c6e] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#214b86]"
            >
              Open search
            </Link>
          </article>
          ...
          <article className="rounded-2xl border border-slate-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-600 dark:text-gray-400">
              Resume the default booking flow or jump into the currently
              selected train details.
            </p>
            <Link
              href="/booking"
              className="mt-5 inline-flex rounded-xl bg-orange-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-orange-600"
            >
              Open booking
            </Link>
          </article>
